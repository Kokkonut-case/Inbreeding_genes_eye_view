...
We use these equations to produce Figures 1.

## Building Figure 1a-b
...
Within_organism_conflict_females <-
  ...
  mutate(Case = case_when(c_m == 1 & c_f == 1 & F_m == F_f ~ "Autosome",
                          ...
                          c_m == 0 & c_f == 1 & F_m == 1 & F_f == 1 ~ "Cyto. (m)",
                          c_m == 1 & c_f == 0 & F_m == 1 & F_f == 1 ~ "Cyto. (p)"),
...
figure_1_thresholds <- 
  ...
  mutate(D = case_when(Sex == "A. Female within-organism conflict" ~ (r*c_m*F_m + r*c_m) / 
  ...
  mutate(Case = case_when(c_m == 1 & c_f == 1 & F_m == F_f ~ "Autosome",
                          c_m == 0.5 & c_f == 1 & F_m == 1 ~ "X",
                          c_m == 1 & c_f == 0.5 & F_f == 1 ~ "Z",
                          c_m == 0 & c_f == 1 & F_m == 0 & F_f == 1 & 
                            Sex == "A. Female within-organism conflict" ~ "W & Cyto. (m)",
                          c_m == 1 & c_f == 0 & F_m == 1 & F_f == 0 & 
                            Sex == "B. Male within-organism conflict"~ "Y & Cyto. (p)",
                          c_m == 0 & c_f == 1 & F_m == 1 & F_f == 1 & 
                            Sex == "B. Male within-organism conflict" ~ "Cyto. (m)",
                          c_m == 1 & c_f == 0 & F_m == 1 & F_f == 1 & 
                            Sex == "A. Female within-organism conflict" ~ "Cyto. (p)"),
         Inbreeding_speed = case_when(Case == "X" & F_f == 0 |
                                        Case == "Z" & F_m == 0 |
...
Fig_1ab <-
  bind_rows(
    Within_organism_conflict_females %>% 
      mutate(Sex = "A. Female within-organism conflict"),
    
    Within_organism_conflict_males %>% 
      mutate(Sex = "B. Male within-organism conflict")
  ) %>% 
  ggplot(aes(x = r, y = D)) +
  geom_blank() +
  ...
  guides(fill="none") +
  geom_textline(data = threshold_1 %>% 
                  filter(Sex == "A. Female within-organism conflict"), 
                aes(x = r, y = D, group = Case, label = Case), size = 4, vjust = -0.25) +
  geom_textline(data = threshold_2 %>% filter(Sex == "A. Female within-organism conflict"), 
                aes(x = r, y = D, group = Case, label = Case), size = 4, vjust = 1.2) +
  geom_textline(data = threshold_1 %>% filter(Sex == "B. Male within-organism conflict"), 
                aes(x = r, y = D, group = Case, label = Case), size = 4, vjust = -0.25) +
  geom_textline(data = threshold_2 %>% filter(Sex == "B. Male within-organism conflict"), 
                aes(x = r, y = D, group = Case, label = Case), size = 4, vjust = 1.2) +
  coord_fixed() +
  facet_wrap(~Sex) +
  labs(y = ~delta~'(inbreeding depression)', 
       x = '_r_ (genealogical relatedness)') +
...
sexual_conflict_plot <-
  ...
  mutate(Case = case_when(str_detect(Case, "W") ~ "W",
         ...
         D = case_when(str_detect(Case, "p") ~ D - 0.000001,
                       .default = D)) %>%
  ggplot(aes(x = 1-D, y = reorder(Case, -D), group = Case)) +
  geom_line(size = 1) +
  geom_point(aes(fill = Sex), shape = 21, size = 9, stroke = 1) +
  scale_fill_manual(values = c("#d0e2af", "pink", "aliceblue")) +
  annotate("text", x = 0, y = 1, label = "0") +
  annotate("text", x = 0, y = 2, label = "0") +
  annotate("text", x = 0.2, y = 3, label = "1/5") +
  annotate("text", x = 0.5, y = 3, label = "1/2") +
  annotate("text", x = 0.33, y = 4, label = "1/3") +
  annotate("text", x = 0.67, y = 4, label = "2/3") +
  annotate("text", x = 0.5, y = 5, label = "1/2") +
  annotate("text", x = 0.8, y = 5, label = "4/5") +
  annotate("text", x = 1, y = 6, label = "1") +
  annotate("text", x = 1, y = 7, label = "1") +
  labs(x = ~delta~'(inbreeding depression)',
       y = "Genomic location",
       title = "C . Sexual conflict over full-sibling inbreeding") +
...
#| fig-width: 8
#| fig-height: 9
Fig_1ab / sexual_conflict_plot + plot_layout(heights = c(2,1.5))
```
...
parameters_fine_X <-
  ...
  expand_grid(
    chromosome = "X",
    v = 80,
    D = seq(-0.5, -0.67, length = resolution_fine), # inbreeding depression
    refractory_period_prop_cohort_alive = seq(0.8, 0.999, length = 15)
  ) %>% 
  ...
  expand_grid(
    chromosome = "X",
    v = 80,
    D = seq(-0.2, -0.33, length = resolution_fine), # inbreeding depression
    refractory_period_prop_cohort_alive = seq(0.8, 0.999, length = 15)
  ) %>% 
  full_join(tibble(chromosome = "X",
                   sex_expressed = 1,
...
parameters_fine_Z <-
  ...
  expand_grid(
    chromosome = "Z",
    v = 80,
    D = seq(-0.67, -0.8, length = resolution_fine), # inbreeding depression
    refractory_period_prop_cohort_alive = seq(0.8, 0.999, length = 15)
  ) %>% 
  full_join(tibble(chromosome = "Z",
   ...
   expand_grid(
    chromosome = "Z",
    v = 80,
    D = seq(-0.33, -0.5, length = resolution_fine), # inbreeding depression
    refractory_period_prop_cohort_alive = seq(0.8, 0.999, length = 15)
  ) %>% 
  full_join(tibble(chromosome = "Z",
                   sex_expressed = 1,
...
if(!file.exists("parameters/parameters_fine_X.txt")){
  parameters_fine_X %>% write.table("parameters/parameters_fine_X.txt")
  parameters_fine_Z %>% write.table("parameters/parameters_fine_Z.txt")
  parameters_fine_AX %>% write.table("parameters/parameters_fine_AX.txt")
  parameters_fine_AZ %>% write.table("parameters/parameters_fine_AZ.txt")
}
```
...
In practice, we ran the simulations on JGU's Mogon computing cluster. See the `HPC_inbreeding_script.R` and the batch script `run_inbreeding_sim`. 
...
if(!file.exists("results/autosome_results_fine_AX.csv")){
  results <- 
    map_dfr(files, results_reader) %>% 
    rename(time = V1,
           I_prop = V2,
    ...
    mutate(parameter_space_ID = str_remove(parameter_space_ID, "rowID_"),
           parameter_space_ID = as.integer(str_remove(parameter_space_ID, "A.csv"))) %>% 
    left_join(parameters_fine_AX, by = "parameter_space_ID")
  
  write_csv(results, "results/autosome_results_fine_AX.csv")
}else{
  autosome_results_fine_AX <- read_delim("results/autosome_results_fine_AX.csv")
}

if(!file.exists("results/autosome_results_fine_AZ.csv")){
  results <- 
    map_dfr(files, results_reader) %>% 
    rename(time = V1,
           I_prop = V2,
    ...
    mutate(parameter_space_ID = str_remove(parameter_space_ID, "rowID_"),
           parameter_space_ID = as.integer(str_remove(parameter_space_ID, "A.csv"))) %>% 
    left_join(parameters_fine_AZ, by = "parameter_space_ID")
  
  write_csv(results, "results/autosome_results_fine_AZ.csv")
}else{
  autosome_results_fine_AZ <- read_delim("results/autosome_results_fine_AZ.csv")
}
...
if(!file.exists("results/X_results_fine.csv")){
  results <- 
    ...
    mutate(parameter_space_ID = str_remove(parameter_space_ID, "rowID_"),
           parameter_space_ID = as.integer(str_remove(parameter_space_ID, "X.csv"))) %>% 
    left_join(parameters_fine_X, by = "parameter_space_ID")
...
if(!file.exists("results/Z_results_fine.csv")){
  results <- 
    ...
    mutate(parameter_space_ID = str_remove(parameter_space_ID, "rowID_"),
           parameter_space_ID = as.integer(str_remove(parameter_space_ID, "Z.csv"))) %>% 
    left_join(parameters_fine_Z, by = "parameter_space_ID")
...
Now stitch all the relevant results together so that we can feed it to `ggplot` as one dataframe.
...
Sunset <- c("#b0cbe7", "#eba07e", "#fef7c7", "#a8554e", "white")
...
Fig2_data %>% 
  ...
  labs(x = ~delta~'(inbreeding depression)',
       y = "Male refractory period (prob. outbred life has ended)",
...
A_data_dominance <-
  ...
  select(I_prop, 
         ...
         refractory_period_prop_cohort_alive,
         dominance) %>% 
  pivot_wider(id_cols = c(D_prop, refractory_period_prop_cohort_alive, sex_expressed, dominance), 
              names_from = v_cat, values_from = I_prop) %>% 
  mutate(Result = case_when(Low >= 0.9 & High >= 0.9 ~ "Fixes with low &\nhigh encounter rate",
                            Low < 0.9 & High >= 0.9 ~ "Fixes with high\nencounter rate",
                            Low >= 0.9 & High < 0.9 ~ "Fixes with low\nencounter rate",
...
AX_heatmap_fine <-
  ...
  labs(x = ~delta~'(inbreeding depression)',
       y = "Male refractory period (prob. outbred life has ended)",
       fill = "Fate of _I_ allele") +
  facet_wrap(chromosome ~ sex_expressed, nrow = 2,
